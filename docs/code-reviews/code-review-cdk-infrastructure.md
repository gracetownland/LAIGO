...
**Scope Coverage Note:** All 7 stacks in `cdk/lib/` are within scope (amplify-stack, api-stack, cicd-stack, database-stack, dbFlow-stack, vpc-stack, waf-stack). Findings concentrate on `api-stack.ts` (largest file, most issues), `vpc-stack.ts`, `dbFlow-stack.ts`, and `database-stack.ts`. `cicd-stack.ts` findings are covered in the Well-Architected review (WA-M5, WA-M8). `amplify-stack.ts` S3 concerns are covered in the S3 Best Practices review. `waf-stack.ts` is noted positively (no issues found).
...
```typescript
// ❌ Problem: single 2609-line file defining everything
export class ApiStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ApiStackProps) {
    ...
    // WAF (150 lines)
    // 12+ Lambda functions (800 lines)
    // DynamoDB tables (200 lines)
    // S3 buckets (150 lines)
    // EventBridge + SQS (200 lines)
    // ... 2609 lines total
  }
}

// ✅ Fix: decompose into focused stacks with cross-stack references
...
export class DataStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DataStackProps) {
    super(scope, id, props);
    this.chatHistoryTable = new dynamodb.Table(this, "ChatHistory", { /* ... */ });
    this.audioStorageBucket = new s3.Bucket(this, "AudioStorage", { /* ... */ });
  }
}
...
const computeStack = new ComputeStack(app, "ComputeStack", {
  ...
  // Cross-stack references via props
});
```
