...
### H1. SQL injection pattern in `createAppUsers()` via string interpolation (latent)
- **Status:** ✅ Fixed
...
- **Impact:** Currently not exploitable (hex-only passwords). But the pattern is a latent vulnerability that becomes critical if password generation logic changes.
...
### H2. `case_reviewers` table lacks `ON DELETE CASCADE` — orphan risk
- **Status:** ✅ Fixed
...
- **Impact:** Orphan `case_reviewers` rows accumulate if cases are deleted outside the instructor handler, and direct SQL case deletion fails with a foreign key violation.
...
### H3. No index on `cases.student_id` — slow queries for student dashboards
- **Status:** ✅ Fixed
- **File:** `migrations/000_initial_schema.js`
...
```sql
-- Current schema: no index on student_id
CREATE TABLE cases (
...
-- Missing: CREATE INDEX idx_cases_student_id ON cases(student_id);
```
...
- **Impact:** A user could end up with an empty roles array if the application-level check is bypassed (e.g., direct SQL update), causing authorization failures or undefined behavior.
...
- [ ] S3 Best Practices (bucket configurations, access policies, encryption, lifecycle rules, versioning)
